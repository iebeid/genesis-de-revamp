...
// Create a list from a Stickynote that references a Node
    GenesisList(StickyNote sn) {
        Utility.DEBUG = false;
        if (Utility.DEBUG) {
            Utility.entering("GenesisList(StickyNote)");
        }
...
// Create a list from a Node: reuse the first MN if v is a MN
    GenesisList(Node v) {
        if (Utility.DEBUG) {
            Utility.entering("GenesisList( Node v )");
        }
        init();
        if (v == null) {
            p.head = new MetaNode();
    ...
    GenesisList(GenesisList gl) {
        if (Utility.DEBUG) {
            Utility.entering("GenesisList( GenesisList v )");
        }
        ...
        if (Utility.DEBUG) {
            Utility.leaving("GenesisList( GenesisList v )");
        }
    }
...
//   p.point = null;
    ...
    StickyNote get() {
        if (Utility.DEBUG) {
            Utility.entering("get with" + p.point);
        }
        ...
        if (Utility.DEBUG) {
            Utility.leaving("get with " + v.addr());
        }
        return v;
    }
...
//   Utility.println("Leaving insert with:");
        //  printNodes();
        if (Utility.DEBUG) {
            Utility.leaving("insert with", this);
        }
    }
...
//   Utility.println("-------------------------");
    ...
    // DN is the node in the destination list
    {
        StickyNote v;
        if (Utility.DEBUG) {
            Utility.entering("GenesisList::printNodes");
        }
        // printDesc(); 
        while (SN != null) {
        ...
        if (Utility.DEBUG) {
            Utility.leaving("GenesisList::printNodes");
        }
    ...
    void move() {
        //Utility.DEBUG = true;
        if (Utility.DEBUG) {
            Utility.entering("GenesisList::move");
        }
        //skipMetaNodes();
/*
         if (p.point == null) {
...
//  printNodes(p.head);
        if (Utility.DEBUG) {
            Utility.leaving("GenesisList::move");
        }
    }
...
// added by Wes Potts 4/8/04
    void move(int n) {
        if (Utility.DEBUG) {
            Utility.entering("GenesisList:move(" + n + ")");
        }
        if (n < 0 || n > size()) {
...
// added by Wes Potts 4/8/04
    void moveTo(int n) {
        ...
        move(n);
        if (Utility.DEBUG) {
            Utility.leaving("goToItem( int n )");
        }
    }
...
////////////////////////////////////////////////////
    ...
    boolean off() { // Loo
        if (Utility.DEBUG) {
            Utility.entering("GenesisList::off");
        }
        boolean ans = (p.head == null)
                || ((p.point != null) && (p.point.right() == null));

        if (Utility.DEBUG) {
            Utility.leaving("off with", ans);
        }
        return ans;
    ...
    boolean on() {
        boolean on;
        if (Utility.DEBUG) {
            Utility.entering("GenesisList::on");
        }
        // on = (p.head != null) &&
        // ((p.point == null) || (p.point.right() != null));
        on = p.point.right != null;
        if (Utility.DEBUG) {
            Utility.leaving("GenesisList::on with ", on);
        }
        return on;
    }
...
//      return  on() && p.point != null && p.point.right == p.head; // ljm: 3/11/07
    ...
    boolean atLast() {
        if (Utility.DEBUG) {
            Utility.entering("GenesisList::atLast");
        }
        boolean ans = false;
        Node t;
        /*
        ...
        if (Utility.DEBUG) {
            Utility.leaving("GenesisList::atLast with ", ans);
        }
        return ans;
    }
...
/*
    ...
    public String implode() {
        ...
        if (on()) {
            result = result + "...rest of string truncated";
        }
        moveTo(position);
        // System.out.println ("Position is now" + position);
        return result;
    ...
    public String toString() {
        ...
        if (Utility.DEBUG) {
            Utility.entering("GenesisList::toString");
        }
        // printNodes(p.head);
        recurseLimit--;
        if (recurseLimit == 0) {
...
// printNodes();
        ...
        while (on() && maxprintLength > 0) {
            ...
            String current = "";
            if (i == position) {
                current = currentIndicator; //currentIndicator ;
            }
            if (true) {
        ...
        if (Utility.DEBUG) {
            Utility.leaving("GenesisList::toString");
        }
        return result;
    }
    ...
    private Node skipMetaNodes(Node n) {
        if (n != null) {
            while (n.right() instanceof MetaNode) {
                n = n.right();
            }
        }
        return n;
...
/*
    ...
    public static void quoteStrings(boolean q) {
        myQuoteStrings = q;
    }
...
// Schedule this for  simplification
    GenesisList copy() {
        if (Utility.DEBUG) {
            Utility.entering("GenesisList::copy");
        }
        int position = pos();
   ...
   /*while (on()) {
        ...
        if (Utility.DEBUG) {
            Utility.leaving("GenesisList::copy");
        }
        return gl;
    }
...
//  with (A.P)^)
        VR = p.head == null;
        if (!VR) {
            Utility.println("List has no descriptor");
        }

        VR = p.head == null;
        if (!VR) {
            Utility.println("List current not on a node");
        }
        VR = noLoops();
        if (!VR) {
            Utility.println("List seemingly has a loop");
        }
        return VR;
    }
    ...
    @Override
    public String toXML() {
        String parent = "<Parent>"+p.parent+"</Parent>";
        Node n = p.head;
        String firstValue = n.toXML();
        return "<ListValue>"+parent+firstValue+"</ListValue>";
    }
}
